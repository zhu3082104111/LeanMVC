...
// 【外协加工调度单号】链接按下的事件
var showSupOrderInfo = function (id) {
    parent.addTab("外协加工调度单", "Purchase/SupplierOrder/SupplierOrderForEdit?supplierOrderId=" + id);
};

// 【合并排产】按钮按下的事件
var showSupplierSchedulingFun = function () {
    ...
    //传递给外协排产界面
    toSupplierScheduling(schedulingObject, deptName);

}

// 实现跳转，进入外购排产页面
var toSupplierScheduling = function (schedulingObject, deptName) {
    parent.addTab("外协排产", "/Purchase/SupplierScheduling/SupplierScheduling?schedulingObject=" + schedulingObject + "&deptName=" + deptName);
};
/***********operations end***************/
...
var TABLE_COLUMNS = [
    [//第一行
        ...
        ["PdProcName", "本道<br>工序名", 80, 2, null, false],
    ...
    [//第二行
        ...
        ["SupplierOrderList", "外协加工单号", 120, null, null, false, { type: "validatebox" }, function (value, row, index) {
            var str = "";
            for (var i = 0, len = value.length; i < len; i++) {
                if (i == len) {
                    str += "<a href='javascript:;'  onclick='showSupOrderInfo(" + "\"" + value[i]["SuppOrderID"] + "\"" + ");'>" + value[i]["SuppOrderID"] + "</a>";
                } else {
                    str += "<a href='javascript:;'  onclick='showSupOrderInfo(" + "\"" + value[i]["SuppOrderID"] + "\"" + ");'>" + value[i]["SuppOrderID"] + "</a><br/>";
                }
        ...
        ["WaitingQuantity", "外协需求量", 80, null, null, false, null]
    ]
];
...
$(function () {
    ...
    Common.Functions.createDataGrid({
        ...
        // 获取数据的地址,必须
        url: "/Purchase/SupplierInstructionList/Query",
        // 列属性,必须
