...
$(_1e3).html($("<div class=\"panel-loading\"></div>").html(opts.loadingMessage));
    ...
    if (opts.onBeforeLoadFun) {
        var queryParam = {};
        opts.onBeforeLoadFun.call(this, queryParam,opts);
        $.ajax({
            url: opts.href,
            cache: false,
            data: queryParam,
...
opts.top = ($(window)._outerHeight() - _231) / 2 + $(document).scrollTop();//old
//opts.top = ($("body")[0].clientHeight - _231) / 2 + $(document).scrollTop();//new 20131225 dxj
}
if(_22f){
_223(_22e);
}
...
//opts.tab=$("<li></li>").appendTo(tabs);//old
/******************dxj 20131228 start******************/
var selectedTab = $(_2e9).tabs("getSelected");
if (typeof $(_2e9).tabs("options").extend != "undefined" && selectedTab) {
    var ind = $(_2e9).tabs("getTabIndex", selectedTab);
    opts.tab = $("<li></li>").insertAfter(tabs.children(":eq(" + ind + ")"));//new dxj
} else {
...
var pp;//=$("<div></div>").appendTo($(_2ed).children("div.tabs-panels"));//old
    /******************dxj 20131228 start******************/
var selectedTab = $(_2ed).tabs("getSelected");
if (typeof $(_2ed).tabs("options").extend != "undefined" && selectedTab) {
    var ind = $(_2ed).tabs("getTabIndex", selectedTab);
    pp = $("<div></div>").insertAfter($(_2ed).children("div.tabs-panels").children(":eq(" + ind + ")"));//new dxj
    $.data(_2ed, "tabs").tabs = tabs.slice(0, ind + 1).concat([pp]).concat(tabs.slice(ind + 1));
    _2ee.selectedIndex = ind + 1;
} else {
    pp = $("<div></div>").appendTo($(_2ed).children("div.tabs-panels"));
    tabs.push(pp);
}
...
_2cf(_2ed);
if (_2ee.selected) {
//_2ef(_2ed,tabs.length-1);//old
    /*****************dxj 20131228 start******************/
    if (typeof _2ee.selectedIndex != "undefined") {
        _2ef(_2ed, _2ee.selectedIndex);
    } else {
...
function init(_7c5){//日历的输入框
$(_7c5).addClass("combo-f").hide();
var span = $("<span class=\"combo\">" + "<input type=\"text\" class=\"combo-text readOnly_class\" autocomplete=\"off\" />" + "<span><span class=\"combo-arrow\"></span></span>" + "<input type=\"hidden\" class=\"combo-value\" />" + "</span>").insertAfter(_7c5);
...
if (typeof opts.clearable !="undefined" && opts.clearable === false) {
} else {
    var _dxj_1 = $("<a href=\"javascript:void(0)\" class=\"datebox-clear\"></a>").html(opts.clearText).appendTo(_898);
    _dxj_1.on("click", function () {
        //$(_892).val("");
